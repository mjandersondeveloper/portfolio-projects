...
std::string mutateA(std::string Origin) {
  if(Origin.length() > 0) {
      int randomPosition = rand() % Origin.length();
      Origin.erase(Origin.begin()+randomPosition); 
  } 
  return Origin;
}
...
std::string mutateB(std::string Origin) {
  ...
  if(Origin.length() > 0) {
    int numberOfInsertions = rand() % 10;
    for (int i = 0; i < numberOfInsertions; i++) {
      int position = rand() % Origin.length();
      int insertChar = rand() % 25;
      Origin.insert(position,alphabet[insertChar]);
    }
  }
  return Origin;
}
