# -*- coding: utf-8 -*-
"""
knapsack.py - CS6515, Intro to Graduate Algorithms, Fall 2023
...
def initTable(numItems, maxWeight):
    ...
    T = [[0 for _ in range(0, maxWeight + 1)] for _ in range(0, numItems + 1)]
    return T
...
def subProblem(T, weight, itemIDX, itemWeight, itemValue):
    ...
    x = T[itemIDX - 1][weight]
    if itemWeight <= weight:
        T[itemIDX][weight] = max(x, itemValue + T[itemIDX - 1][weight - itemWeight])
    else:
        T[itemIDX][weight] = x
        
    subproblem_value = T[itemIDX][weight]
...
def buildResultList(T, itemsDict, maxWeight):
    ...
    for i in reversed(range(1, len(itemsDict) + 1)):
        itemWeight = T[i][maxWeight]
        if itemWeight <= 0:
            break
        elif itemWeight != T[i - 1][maxWeight]:
            result.append(itemsDict[i])
            maxWeight -= itemsDict[i][1]
    return result
